...
std::mutex mtx;
template <typename C>
class LockWorker: public Worker<C> {
public:
    LockWorker(int _id, C* _count) : Worker<C>(_id, _count) {};
    void operator() () {
        for (int i = 0; i < 100000; i++) {
...
int main()
{
    std::cout << "Non-lock thread :" << std::endl;
    int count1 = 0;
    run<NonLockWorker<int>, int>(&count1);
    std::cout << std::endl;

    std::cout << "Lock thread :" << std::endl;
    int count2 = 0;
    run<LockWorker<int>, int>(&count2);
    std::cout << std::endl;
